...
router.get('/', async (req, res, next) => {
  try {
    ...
    // Attach live product counts. On the storefront (all !== 'true') only
    // count visible products so hidden ones don't inflate category counts.
    const countMatch = includeInactive ? {} : { visible: { $ne: false } };
    const countPipeline = [];
    if (Object.keys(countMatch).length) countPipeline.push({ $match: countMatch });
    countPipeline.push({ $group: { _id: '$categoryId', count: { $sum: 1 } } });
